...
trait GeneratorTrait
{
    ...
    protected function execute(array $params): void
    {
        ...
        if (!$this->getOption('force') && $isFile) {
            CLI::error('File exist: ' . realpath($path) . ' ❌ ', 'light_gray', 'red');
            CLI::newLine();
        ...
        if (!writeFile($path, $this->buildContent($class, $this->replace ?? [], $this->data ?? []))) {
            ...
            CLI::error('File Error: ' . realpath($path) . ' ❌ ', 'light_gray', 'red');
            CLI::newLine();
            return;
        }

        if ($this->getOption('force') && $isFile) {
            CLI::write('File Over write: ' . realpath($path) . ' 🤙', 'yellow');
            CLI::newLine();
            return;
        }

        CLI::write('File Create: ' . realpath($path) . ' 🤙', 'green');
        CLI::newLine();
    }
    ...
    {
        $templatePath = (is_file($this->template)) ?  $this->template : __DIR__ . "/Commands/Generators/Template/{$this->template}";
    ...
    /**
     * Perform replacements in a template using search and replace arrays and return the modified template.
     *
     * @param string $class The fully qualified class name.
     * @param array $search An array of search patterns.
     * @param array $replace An array of replacement strings.
    ...
    protected function parseTemplate(string $class, array $replacements = [], array $data = []): string
    {
        ...
        // Perform the replacements on the template and return it.
        return str_replace(array_keys($replacements), $replacements, $this->renderTemplate($data ?? $this->data ?? []));
    }
    ...
    protected function buildPath(string $class): string
    {
        ...
        $base = [$namespace];
        if (!$base = reset($base)) {
            CLI::error('Namespace not defined: ' . $namespace . ' ❌ ', 'light_gray', 'red');
            CLI::newLine();
    ...
    /**
     * Build the file path based on the class and namespace.
     *
     * @param string $class The fully qualified class name.
     * @return string The file path.
     * @throws Exception If there is an issue with the namespace or class.
     */
    // protected function buildPath(string $class): string
    // {
    //     // Validate the namespace.
    //     $namespace = trim(str_replace('/', '\\', $this->getOption('namespace') ?? $this->namespace ?? APP_NAMESPACE), '\\');
    //     if (empty($namespace) || !class_exists($class)) {
    //         throw new \Exception('Invalid namespace or class provided.');
    //     }

    //     // Build the path and return it.
    //     $base = realpath($namespace) ?: $namespace;
    //     $file = $base . DIRECTORY_SEPARATOR . str_replace('\\', DIRECTORY_SEPARATOR, trim(str_replace($namespace . '\\', '', $class), '\\')) . '.php';
    ...
    //     // $namespace = trim(
    //     //     str_replace('/', '\\', $this->getOption('namespace') ?? $this->namespace ?? APP_NAMESPACE),
    //     //     '\\'
    //     // );
    ...
    //     // return trim(str_replace($namespace . '\\', '', $class), '\\') . '.php';
    // }


    // dd(
    //     $base,
    //     trim(str_replace($namespace . '\\', '', $class), '\\') . '.php'
    // );
