Raxm provides a variety of lifecycle hooks that allow you to execute code at specific points during a component's lifecycle. These hooks enable you to perform actions before or after particular events, such as initializing the component, updating properties, or rendering the template.
...
In a standard PHP class, a constructor (`__construct()`) takes in outside parameters and initializes the object's state. However, in Raxm, you use the `mount()` method for accepting parameters and initializing the state of your component.

Raxm components don't use `__construct()` because Raxm components are _re-constructed_ on subsequent network requests, and we only want to initialize the component once when it is first created.
...
> Raxm allows you to resolve dependencies out of [Raxm's service container](https://laravel.com/docs/container#automatic-injection) by type-hinting method parameters on lifecycle hooks.

The `mount()` method is a crucial part of using Raxm. The following documentation provides further examples of using the `mount()` method to accomplish common tasks:
...
For these cases, Raxm provides a `boot()` method where you can write component setup code that you intend to run every single time the component class is booted: both on initialization and on subsequent requests.
...
You can use this technique to have complete control over initializing a component property in your Raxm component.

> [!tip] Most of the time, you can use a computed property instead
> The technique used above is powerful; however, it's often better to use [Raxm's computed properties](/docs/computed-properties) to solve this use case.
...
Client-side users can update public properties in many different ways, most commonly by modifying an input with `axm:model` on it.

Raxm provides convenient hooks to intercept the updating of a public property so that you can validate or authorize a value before it's set, or ensure a property is set in a given format.
...
Because you are often targeting a specific property when using update hooks, Raxm allows you to specify the property name directly as part of the method name. Here's the same example from above but rewritten utilizing this technique:
...
The terms "dehydrate" and "hydrate" refer to a Raxm component being serialized to JSON for the client-side and then unserialized back into a PHP on the subsequent request.

We often use the terms "hydrate" and "dehydrate" to refer to this process throughout Raxm's codebase and the documentation. If you'd like more clarity on these terms, you can learn more by [consulting our hydration documentation](/docs/hydration).
...
To avoid multiple traits conflicting with each other when declaring lifecycle hook methods, Raxm supports prefixing hook methods with the _camelCased_ name of the current trait declaring them.
