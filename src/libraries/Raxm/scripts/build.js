...
build({
    entryPoints: [`js/index.js`],
    outfile: `dist/raxm.min.js`,
    bundle: true,
    minify: true,
    platform: 'browser',
    define: { CDN: true },
}).then(() => {
    outputSize(`dist/raxm.min.js`)
})

function build(options) {
    options.define || (options.define = {})

    // options.define['raxm_VERSION'] = `'${getFromPackageDotJson('alpinejs', 'version')}'`
